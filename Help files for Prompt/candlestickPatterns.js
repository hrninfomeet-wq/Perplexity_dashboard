...
class CandlestickPatterns {
    ...
    isHammer(candle) {
        ...
        // 3. Body should be in upper half of range
    ...
    calculateHammerConfidence(candle, candles, index) {
        ...
        // Increase confidence if at support level or after downtrend
        if (index > 5) {
            const recentTrend = this.getRecentTrend(candles, index, 5);
            if (recentTrend === 'bearish') {
                confidence += 0.1; // Higher confidence in downtrend
            }
        }
        
        // Volume confirmation would increase confidence (if available)
    ...
    isShootingStar(candle) {
        ...
        // 1. Upper shadow at least 2x body size
        // 2. Lower shadow should be small (< 0.5x body)
        // 3. Body should be in lower half of range
    ...
    calculateShootingStarConfidence(candle, candles, index) {
        ...
        // Increase confidence if at resistance level or after uptrend
        if (index > 5) {
            const recentTrend = this.getRecentTrend(candles, index, 5);
            if (recentTrend === 'bullish') {
                confidence += 0.1; // Higher confidence in uptrend
            }
        }
    ...
    // (Harami, Piercing Line, Dark Cloud Cover, Three White Soldiers, etc.)
