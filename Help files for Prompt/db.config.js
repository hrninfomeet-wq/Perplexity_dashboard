...
class DatabaseConfig {
    ...
    async connect() {
        try {
            ...
            if (this.connectionAttempts < this.maxRetryAttempts) {
                ...
                throw new Error(`Database connection failed after ${this.maxRetryAttempts} attempts: ${error.message}`);
            }
        }
    }
    ...
    async createIndexes() {
        try {
            if (!this.isConnected) {
                console.log('⚠️ Cannot create indexes: database not connected');
                return false;
            }
            ...
            await userCollection.createIndex({ flattrade_user_id: 1 }, { sparse: true });
            await userCollection.createIndex({ created_at: 1 });

            // Trade collection indexes
            const tradeCollection = mongoose.connection.collection('trades');
            await tradeCollection.createIndex({ user_id: 1 });
            await tradeCollection.createIndex({ symbol: 1 });
            await tradeCollection.createIndex({ timestamp: -1 }); // Descending for recent trades
            await tradeCollection.createIndex({ signal_type: 1 });
            await tradeCollection.createIndex({ user_id: 1, timestamp: -1 }); // Compound index

            // Session collection indexes
            const sessionCollection = mongoose.connection.collection('sessions');
            await sessionCollection.createIndex({ user_id: 1 });
            await sessionCollection.createIndex({ token: 1 }, { unique: true });
            await sessionCollection.createIndex({ expires_at: 1 }, { expireAfterSeconds: 0 }); // TTL index
