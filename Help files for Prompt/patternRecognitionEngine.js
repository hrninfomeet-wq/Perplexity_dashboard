...
class PatternRecognitionEngine {
    ...
    async start() {
        try {
            ...
            console.log(`⚡ Enhanced timeframes: ${Object.keys(TIMEFRAME_CONFIG.SUPPORTED_TIMEFRAMES).join(', ')}`);
    ...
    async detectAllPatterns(symbol, timeframes = null) {
        try {
            const targetTimeframes = timeframes || Object.keys(TIMEFRAME_CONFIG.SUPPORTED_TIMEFRAMES);
    ...
    startPeriodicPatternDetection() {
        Object.entries(TIMEFRAME_CONFIG.UPDATE_INTERVALS).forEach(([timeframe, interval]) => {
    ...
    async detectPatternsForAllSymbols(timeframe) {
        try {
            ...
            const batchSize = TIMEFRAME_CONFIG.PERFORMANCE.BATCH_SIZE;
