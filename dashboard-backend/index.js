...
// Market data routes (with automatic auth handling) - Mount BEFORE auth routes to prevent interception
app.use('/api', 
    AuthMiddleware.marketDataAuth, // Automatically handles auth for market data
    marketDataRoutes
);

// Authentication routes (with unified auth system and database) - Mount AFTER market data routes
app.use('/api', authRoutes);

// Market data routes are now handled by marketDataRoutes.js
// All endpoints (/api/indices, /api/gainers, /api/losers, /api/scalping, /api/btst, /api/fno-analysis) 
// are properly routed through the controller with live Flattrade data
console.log('📊 Market data routes configured through marketDataRoutes.js');

// =============================================================================
// PAPER TRADING API ENDPOINTS (Phase 3A Step 9)
// =============================================================================

// Paper Trading Session Management
app.post('/api/paper-trading/session/start', async (req, res) => {
    try {
        const { userId, startingCapital, strategy, riskLevel, autoExecution, maxPositions, stopLossPercent, takeProfitPercent } = req.body;
        
        console.log('🎯 Starting paper trading session:', { userId, startingCapital, strategy });
        ...
        const session = {
            ...
            currentCapital: startingCapital || 100000,
            strategy: strategy || 'manual',
            riskLevel: riskLevel || 'medium',
            autoExecution: autoExecution || false,
        ...
        // Broadcast session start to WebSocket clients
        ...
        console.error('❌ Error starting paper trading session:', error.message);
        res.status(500).json({
            success: false,
            error: error.message,
...
app.post('/api/paper-trading/session/stop', async (req, res) => {
    try {
        const { sessionId } = req.body;
        
        console.log('🛑 Stopping paper trading session:', sessionId);
        
        // Create mock stop response
        const stopResult = {
            ...
            finalPerformance: {
                ...
                totalPnL: (Math.random() - 0.5) * 10000, // -5000 to +5000
                duration: '2h 15m',
                maxDrawdown: Math.random() * 5 + 1, // 1-6%
                sharpeRatio: Math.random() * 2 + 0.5 // 0.5-2.5
            }
        };

        // Broadcast session stop to WebSocket clients
        const message = JSON.stringify({
        ...
        console.error('❌ Error stopping paper trading session:', error.message);
        res.status(500).json({
            success: false,
            error: error.message,
...
app.get('/api/paper-trading/session/status', async (req, res) => {
    try {
        // Mock session status
        const sessionStatus = {
            hasActiveSession: Math.random() > 0.7, // 30% chance of active session
            currentSession: Math.random() > 0.7 ? {
                sessionId: `session_${Date.now() - 3600000}`,
                status: 'active',
                startedAt: new Date(Date.now() - 3600000).toISOString(),
                duration: '1h 5m',
                currentCapital: 102350,
                totalPnL: 2350,
                openPositions: 3,
                totalTrades: 7
            } : null
